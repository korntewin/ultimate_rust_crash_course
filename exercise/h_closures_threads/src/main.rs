...
fn main() {
    ...
    let total = thread::spawn(move || {expensive_sum(my_vector)});
    ...
    // init sender and receiver channel
    let (tx, rx) = channel::unbounded();
    let rx2 = rx.clone();

    // spawn two thread of receiver
    let h1 = thread::spawn(move || {
        for msg in rx {
            pause_ms(100);
            println!("Child thread 1 receive {}", msg);
        } 
    });
    ...
    let h2 = thread::spawn(move || {
        for msg in rx2 {
            pause_ms(100);
            println!("Child thread 2 receive {}", msg);
        }
    });

    for val in 0..10 {
        tx.send(format!("value = {}", val)).unwrap();
    }
